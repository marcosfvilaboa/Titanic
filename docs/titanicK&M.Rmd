...
Així doncs, els atributs _Cabin_, _Embarked_ i _Age_ contenen dades a tractar.
...
Per omplir els valors buits de l'atribut _Embarked_ ho farem amb el valor més freqüent:

```{r}
sort(table(titanic$Embarked),decreasing=TRUE)
```
On es pot veure que el valor més repetit és "S". 
...
#### Valors extrems
[TO-DO: IdentificaciÃ³ i tractament]

A continuació creem una funció per trobar els valors extrems dins de les variables numèriques. En aquest cas concret en tenim quatre: _Age_, _SibSp_, _Parch_ i _Fare_
```{r}
...
El que es fa en aquesta funció és eliminar les tuples del dataset que contenen outliers en l'atribut d'entrada. Aleshores, aplicant la funció per a tots els atributs desitjats ens permetrà eliminar totes les tuples que contenen outliers.
```{r}
...
Un cop transformat els dataset s'exporta en un ".csv"
